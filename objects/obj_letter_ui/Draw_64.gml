...
draw_set_color(make_color_rgb(245,238,220));
...
for (var i = 0; i < array_length(choices); i++) {
    ...
    // store bounds
    if (i >= array_length(choice_bbox)) array_push(choice_bbox, { xs: bx, ys: by, w: bw, h: bh });
    else {
        choice_bbox[i].xs = bx;
        choice_bbox[i].ys = by;
        choice_bbox[i].w  = bw;
    ...
    // bg
    if (selected_choice == i) draw_set_color(make_color_rgb(190,220,255));
    else draw_set_color(make_color_rgb(230,230,230));
...
if (selected_choice >= 0) draw_set_color(make_color_rgb(100,150,220));
else draw_set_color(make_color_rgb(160,160,160));
